...
const toast = (msg) => {
  ...
  setTimeout(() => bar.classList.add("hidden"), 1600);
};
...
$("#btnConnect")?.addEventListener("click", async () => {
  try {
    ...
    toast("연결 완료");
    try {
      (AppState.onReady || []).forEach((fn) => {
        try {
          if (typeof fn === "function") fn();
        } catch (e) {
          console.error("onReady fn failed", e);
        }
      });
    } catch (e) {}
  } catch (e) {
...
$("#btnExpandAll")?.addEventListener("click", () => {
  $$("#accordionGroups details").forEach((el) => {
    el.open = true;
  });
});
...
async function renderAccordion2() {
  ...
  for (const code of codes) {
    ...
    for (const m of items)
      stat[m.status || "pending"] = (stat[m.status || "pending"] || 0) + 1;
    ...
    sum.className =
      "cursor-pointer list-none select-none px-3 py-2 flex items-center justify-between";
    sum.innerHTML = `
    ...
    for (const m of items) {
      cont.appendChild(makeScoreCard(m));
    }
...
async function renderAccordion(){
  ...
  for (const code of codes){
    const sourceItems = (byG.get(code)||[]).sort((a,b)=>(a.round||0)-(b.round||0)||a.id-b.id);
    const items = sourceItems.filter((m)=>{
      ...
      return text.includes(groupSearchQuery);
    });
  ...
  if (meta){
    if (!groupSearchQuery) meta.textContent = `전체 ${codes.length}개 조 · ${list.length}경기`;
...
function makeScoreCard(m) {
  ...
  card.className =
    "card p-4 score-card flex flex-col hover:shadow-zen transition";
  ...
  card.innerHTML = `
    ...
    <div class="set-row relative border rounded-lg p-3 mt-3 bg-white">
      ...
      <div class="quick-pts hidden absolute left-2 top-full mt-1 z-10 bg-white border rounded-lg shadow p-1 flex gap-1">${(() => {
        const cand = [15, m.points_to_win || 21, m.cap || 25];
        const uniq = [...new Set(cand.filter(Boolean))].sort((a, b) => a - b);
        return uniq
          .map(
            (v) =>
              `<button class="btn px-2 py-1 text-xs" data-quick-val="${v}" ${readOnly || m.status !== "playing" ? "disabled" : ""}>${v}</button>`,
          )
          .join("");
      })()}</div>
    </div>
    ...
    <div class="hint mt-1" data-msg>${
      isDone
        ? "이미 종료된 경기입니다. 변경이 필요하면 관리자에게 문의하세요."
        : st
    }</div>
  `;
  return card;
...
$("#accordionGroups")?.addEventListener("focusout", () => {
  setTimeout(() => {
    if (
      ...
      !$("#accordionGroups")?.contains(document.activeElement)
    ) {
      $$("#accordionGroups .quick-pts").forEach((el) =>
...
$("#accordionGroups")?.addEventListener("click", (e) => {
  ...
  if (wbtn && !wbtn.disabled) {
    ...
    const pts = Math.min(
      parseInt(card.dataset.points || "25", 10),
      parseInt(card.dataset.cap || "25", 10),
    );
    ...
    (async () => {
      try {
        ...
        if ((card.dataset.status || "") !== "pending") {
          toast("이미 시작되었거나 종료된 경기입니다.");
          return;
        }
        const courtNo = card.dataset.courtNo
          ? Number(card.dataset.courtNo)
          : null;
        const courtSeq = card.dataset.courtSeq
          ? Number(card.dataset.courtSeq)
          : null;
        if (!courtNo || !courtSeq) {
          toast("코트 배정이 필요합니다.");
          return;
        }
        const { data: prev } = await AppState.sb
          .from("matches")
          .select("id,status,court_seq")
          .eq("court_no", courtNo)
          .lt("court_seq", courtSeq);
        if ((prev || []).some((x) => x.status !== "done")) {
          toast("이전 경기가 아직 종료되지 않았습니다.");
          return;
        }
        const { data: nowPlaying } = await AppState.sb
          .from("matches")
          .select("id")
          .eq("court_no", courtNo)
          .eq("status", "playing")
...
async function saveOneCard(card) {
  ...
  const A =
    ai.value === "" ? null : Math.max(0, Math.min(cap, Number(ai.value)));
  const B =
    bi.value === "" ? null : Math.max(0, Math.min(cap, Number(bi.value)));
  const setArr =
    typeof A === "number" && typeof B === "number" ? [{ a: A, b: B }] : [];
  const msgEl = card.querySelector("[data-msg]");
  if (setArr.length === 0) {
...
async function renderStandingsPage() {
  ...
  for (const r of gt || []) {
    ...
    byG.set(code, [
      ...(byG.get(code) || []),
      { team_id: r.team_id, name: teamMap.get(r.team_id) || "-" },
    ]);
  }
  ...
  // 조별 결과 모음
  const resRoot = $("#groupResultsAll");
  if (resRoot) resRoot.innerHTML = "";
  const byGm = new Map();
  for (const m of matches) {
    const code = m.groups?.code || "-";
  ...
  for (const code of [...byGm.keys()].sort()) {
    ...
    const box = document.createElement("div");
    box.className = "border rounded-lg p-3 bg-white";
    const head = document.createElement("div");
    head.className = "font-semibold mb-2";
    head.textContent = `${code}조 경기결과`;
    box.appendChild(head);
    items.sort((a, b) => (a.round || 0) - (b.round || 0) || a.id - b.id);
    for (const m of items) {
      const a = labelOrName(m, "a");
      const b = labelOrName(m, "b");
      const aMem = membersStr(m.team_a_id);
      const bMem = membersStr(m.team_b_id);
      const s = (m.set_scores || [])[0] || {};
      const line = document.createElement("div");
      line.className = "flex justify-between text-sm border-t py-1";
...
function computeStandings(byG, matches) {
  ...
  for (const [code, teams] of byG) {
    ...
    for (const m of mByG.get(code) || []) {
      ...
      const A =
        rows.get(m.team_a_id) ||
        rows
          .set(m.team_a_id, {
            team_id: m.team_a_id,
            name: AppState.teamMap.get(m.team_a_id) || "-",
            gp: 0,
            w: 0,
            l: 0,
            pf: 0,
          ...
          .get(m.team_a_id);
      const B =
        rows.get(m.team_b_id) ||
        rows
          .set(m.team_b_id, {
            team_id: m.team_b_id,
            name: AppState.teamMap.get(m.team_b_id) || "-",
            gp: 0,
            w: 0,
            l: 0,
            pf: 0,
    ...
    const arr = [...rows.values()]
      ...
      .sort(
        (x, y) => y.w - x.w || y.diff - x.diff || x.name.localeCompare(y.name),
      );
    out.set(code, arr);
  }
  return out;
...
async function autoConnectFromMeta() {
  ...
  try {
    ...
    console.error("Court Board RT subscribe failed", e);
  }
}
...
async function fetchCourtSnapshot() {
  const { data: list, error } = await AppState.sb
    .from("matches")
    .select(
      "id,court_no,court_seq,status,team_a:team_a_id(name),team_b:team_b_id(name),started_at,last_action_at",
    )
...
async function renderCourtBoard() {
  ...
  try {
    ...
    if (stalled.length) {
      html += `<div class="mb-3 p-2 border rounded-lg bg-amber-50"><strong>⚠ 멈춘 경기</strong> (${STALLED_MINUTES}분 이상 변화 없음): ${stalled
        .map(
          (m) =>
            `코트 ${m.court_no} · ${m.court_seq}번 (${m.team_a?.name || "A"} vs ${m.team_b?.name || "B"})`,
        )
        .join(" , ")}</div>`;
    }
    ...
    for (const [no, arr] of courts) {
      ...
      const next = arr.find(
        (x) =>
          x.status !== "done" && (!playing || x.court_seq > playing.court_seq),
      );
    ...
    console.error("renderCourtBoard", e);
  }
}
// ===== 코트 탭: 코트별 경기순번 나열 =====
async function renderCourtSchedule() {
  ...
  try {
    ...
    const { data: list, error } = await AppState.sb
      .from("matches")
      .select(
        "id,court_no,court_seq,status,team_a:team_a_id(name),team_b:team_b_id(name)",
      )
      .not("court_no", "is", null)
      .order("court_no", { ascending: true })
      .order("court_seq", { ascending: true });
          ...
          .map((m) => {
            const st =
              m.status === "playing"
                ...
                : m.status === "done"
                  ? '<span class="text-slate-500">종료</span>'
